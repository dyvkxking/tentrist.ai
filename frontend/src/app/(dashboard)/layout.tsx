...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  // Connection status - in real app this would come from WebSocket/SSE
  const [connectionStatus, setConnectionStatus] = React.useState<
    "connected" | "disconnected" | "reconnecting"
  >("connected");

  // Wallet data from user or mock
  const wallet = user
    ? {
        address: user.email ? undefined : user.name, // For wallet users, name is address
        balance: "12.5847", // In real app, fetch from blockchain
      }
    : undefined;
